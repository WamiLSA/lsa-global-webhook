...
const PHASE_3_GROUNDWORK = {
  ...
  implementedScope: [
    ...
    'Automation Hub API exposes phase-state, workflow readiness evidence, stable target-opening payloads, and Phase 3 operational diagnostics.'
  ],
  rolloutGuardrails: [
    ...
    'Live Mode remains conservative; Phase 3 automation is limited to internal extraction, routing, matching, notification, target handoff preparation, and operational validation.'
  ]
};
...
const ARTIFACT_COPY = {
  ...
  duplicate_review_queue: {
    ...
    actionLabel: 'Open duplicate review item',
  ...
  provider_matching_results: {
    ...
    actionLabel: 'Open matching results',
...
function hydrateWorkflowStatsFromHistory(workflows, history) {
  for (const wf of workflows) {
    ...
    wf.stats.lastArtifactActionLabel = latest.artifact?.actionLabel || latest.artifact?.targetSync?.openTargetLabel || null;
    wf.stats.lastArtifactTargetSync = latest.artifact?.targetSync || null;
    wf.stats.lastArtifactOpenTargetPayload = latest.artifact?.targetSync?.openTargetPayload || latest.artifact?.openTargetPayload || null;
  }
}
...
function buildTargetReference(artifact = {}, action = null) {
  ...
  if (type === 'provider_extraction_review') {
    ...
    return {
      ...
      openTargetLabel: refs.providerId && !pending ? 'Open provider record' : 'Open provider review panel',
  ...
  if (type === 'kb_formatting_review') {
    ...
    return {
      ...
      destinationUrl: targetUrl,
      openTargetLabel: status === 'published' ? 'Open published KB item' : 'Open KB draft',
  ...
  if (type === 'inbox_helper_panel') {
    ...
    return {
      ...
      destinationUrl: targetUrl,
      openTargetLabel: refs.threadId ? 'Open thread panel' : 'Open inbox helper panel',
  ...
  if (type === 'provider_matching_results') {
    ...
    return {
      ...
      destinationUrl: targetUrl,
      openTargetLabel: status === 'confirmed' ? (refs.providerId ? 'Open provider record' : 'Open confirmed provider target') : 'Open matching results',
  ...
  return {
    ...
    destinationDisplayLabel: artifact.title || toOperatorReferenceLabel(fallbackArtifact, 'Automation review surface'),
    destinationObjectType: moduleName,
    destinationObjectName: artifact.title || 'Automation review surface',
    destinationPanel: artifact.surface || moduleName,
    destinationUrl: targetUrl,
    openTargetLabel: 'Open synchronized item',
    sourceObjectId: fallbackArtifact,
    sourceDisplayLabel: toOperatorReferenceLabel(fallbackArtifact, 'Automation review surface'),
    sourceObjectType: artifact.type || 'automation review artifact',
    traceLabel: `${artifact.title || 'Automation review surface'} → ${moduleName}`,
    verificationHint: pending ? 'Verify the pending automation review surface in its target module.' : 'Target module shows the synchronized automation decision.'
  };
}
...
function buildTargetSync(artifact = {}, action = null, extras = {}) {
  ...
  record.openTargetPayload = buildOpenTargetPayload(record, targetReference);
  return record;
}


function isClosedSynchronizedTargetRecord(record = {}) {
  return record.reviewState === 'closed' && record.syncState === 'synchronized' && !record.pendingReview;
}

function buildOpenTargetPayload(targetSync = {}, targetReference = {}) {
  const reference = targetReference || targetSync.targetReference || {};
  const destinationUrl = targetSync.targetUrl || reference.destinationUrl || '';
  return {
    targetModule: targetSync.targetModule || targetSync.module || '',
    destinationRecordId: targetSync.destinationRecordId || reference.destinationRecordId || '',
    destinationObjectType: targetSync.destinationObjectType || reference.destinationObjectType || '',
    destinationObjectName: targetSync.destinationObjectName || reference.destinationObjectName || targetSync.destinationDisplayLabel || reference.destinationDisplayLabel || '',
    panelLocation: targetSync.destinationPanel || reference.destinationPanel || '',
    destinationUrl,
    syncState: targetSync.syncState || 'awaiting_review',
    reviewState: targetSync.reviewState || 'pending',
    artifactId: targetSync.artifactId || null,
    artifactType: targetSync.artifactType || null,
    openTargetLabel: targetSync.openTargetLabel || reference.openTargetLabel || 'Open target module'
  };
}
...
function buildWorkflow(definition) {
  return {
    ...
    stats: {
      ...
      lastArtifactActionLabel: null,
      lastArtifactTargetSync: null,
      lastArtifactOpenTargetPayload: null
    }
  };
}
...
function buildArtifactChecklist(type, refs) {
  ...
  return ['Open the generated workflow review surface and decide the next staff action.'];
}
...
function createAutomationHub({ getSystemMode, persistencePath } = {}) {
  const defaultState = { notifications: [], history: [], artifacts: [], auditLog: [], targetModuleState: [], workflowStates: {}, idempotencyKeys: [] };
  ...
  const idempotencyKeys = safeArray(persistedState.idempotencyKeys);
  ...
  function persist() {
    persistState(persistencePath, { notifications, history, artifacts, auditLog, targetModuleState, workflowStates, idempotencyKeys });
  }
  function buildActionOperationKey(artifact, action, lifecycle = null, explicitKey = '') {
    if (explicitKey) return explicitKey;
    const currentReviewState = lifecycle?.reviewState || getArtifactLifecycle(artifact).reviewState;
    const currentSyncState = artifact.targetSync?.syncState || (currentReviewState === 'closed' ? 'synchronized' : 'awaiting_review');
    const destinationRecordId = artifact.targetSync?.destinationRecordId || buildTargetReference(artifact, action).destinationRecordId || '';
    const transitionVersion = Number.isFinite(Number(artifact.transitionVersion)) ? Number(artifact.transitionVersion) : 0;
    return [
      artifact.id,
      action.id,
      action.targetModule || artifact.module || '',
      destinationRecordId,
      currentReviewState,
      currentSyncState,
      transitionVersion
  ...
  function useIdempotencyKey(key) {
    if (!key) return false;
    if (idempotencyKeys.includes(key)) return true;
    idempotencyKeys.unshift(key);
    if (idempotencyKeys.length > 2000) idempotencyKeys.length = 2000;
    return false;
  }
  ...
  function getNotificationGroupKey(message, level, details = {}) {
    const notificationType = details.notificationType || (message.includes('decision recorded') ? 'decision_recorded' : message.includes('lifecycle updated') ? 'lifecycle_updated' : message.includes('Automation completed') ? 'automation_completed' : 'automation_notice');
    if (details.artifactId) return [notificationType, details.artifactId, details.nextStatus || details.statusLabel || details.runId || 'state'].join(':');
    if (details.runId) return [notificationType, details.runId, details.workflowId || 'workflow'].join(':');
    return [notificationType, details.workflowId || 'workflow', details.artifactType || 'artifact', details.nextStatus || details.statusLabel || details.staffReviewSurface || details.triggerType || level].join(':');
  }

  function addNotification(message, level = 'info', details = {}) {
    const notificationType = details.notificationType || (message.includes('decision recorded') ? 'decision_recorded' : message.includes('lifecycle updated') ? 'lifecycle_updated' : message.includes('Automation completed') ? 'automation_completed' : 'automation_notice');
    const groupingKey = details.groupingKey || getNotificationGroupKey(message, level, { ...details, notificationType });
    notifications.unshift({ id: crypto.randomUUID(), level, message, details: { ...details, notificationType, groupingKey }, groupingKey, notificationType, createdAt: nowIso() });
  ...
  function syncTargetModuleState(artifact, decisionEntry) {
    ...
    const sameArtifactActiveIndex = targetModuleState.findIndex(item => item.artifactId === artifact.id && !isClosedSynchronizedTargetRecord(item));
    const sameKeyActiveIndex = targetModuleState.findIndex(item => item.key === record.key && !isClosedSynchronizedTargetRecord(item));
    const sameKeyClosedIndex = targetModuleState.findIndex(item => item.key === record.key && isClosedSynchronizedTargetRecord(item));
    const nextIsClosedSync = isClosedSynchronizedTargetRecord(nextRecord);
    const nextIsActiveReview = !nextIsClosedSync;
    const existingSameKeyIsClosedSync = sameKeyClosedIndex >= 0;
    ...
    } else if (sameArtifactActiveIndex >= 0) {
      targetModuleState[sameArtifactActiveIndex] = nextRecord;
    } else if (nextIsActiveReview && existingSameKeyIsClosedSync) {
      targetModuleState.unshift(nextRecord);
    } else if (sameKeyActiveIndex >= 0) {
      targetModuleState[sameKeyActiveIndex] = nextRecord;
    } else {
      targetModuleState.unshift(nextRecord);
    }
  ...
  function syncHistoryDecision(artifact, decisionEntry) {
    ...
    if (!entry) return;
    const isLifecycleCorrection = decisionEntry.actionCategory === 'lifecycle';
    const actionResult = {
      action: isLifecycleCorrection ? 'record_lifecycle_correction' : 'record_decision',
      ok: true,
    ...
    if (isLifecycleCorrection) {
      entry.lifecycleCorrections = [decisionEntry, ...safeArray(entry.lifecycleCorrections)].slice(0, 25);
      entry.lifecycleUpdatedAt = decisionEntry.decidedAt;
      entry.actionResults = safeArray(entry.actionResults).concat(actionResult);
      return;
    }
  ...
  async function executeWorkflow(wf, triggerType, payload = {}, meta = {}) {
    ...
    wf.stats.lastArtifactActionLabel = ok ? artifact?.actionLabel || artifact?.targetSync?.openTargetLabel || null : null;
    wf.stats.lastArtifactTargetSync = ok ? artifact?.targetSync || null : null;
    wf.stats.lastArtifactOpenTargetPayload = ok ? artifact?.targetSync?.openTargetPayload || artifact?.openTargetPayload || null : null;
    ...
    addNotification(
      ...
      ok ? 'success' : 'error',
      {
        ...
        triggerOwnership: entry.triggerOwnership,
        ...
        targetSync: artifact?.targetSync || null,
        openTargetPayload: artifact?.targetSync?.openTargetPayload || null,
        notificationType: ok ? 'automation_completed' : 'automation_failed'
      }
    );
  ...
  function listNotifications(limit = 30) {
    ...
    for (const notification of notifications) {
      const key = notification.groupingKey || notification.details?.groupingKey || notification.id;
      if (!byKey.has(key)) {
        const item = { ...notification, details: { ...(notification.details || {}) }, repeatCount: 1, repeatedNotificationIds: [notification.id] };
        grouped.push(item);
        byKey.set(key, item);
      } else {
        const existing = byKey.get(key);
        existing.repeatCount += 1;
        existing.repeatedNotificationIds.push(notification.id);
        existing.details = {
          ...existing.details,
          groupedRunIds: [...new Set([...(existing.details.groupedRunIds || []), existing.details.runId, notification.details?.runId].filter(Boolean))],
          latestGroupedAt: existing.createdAt
        };
      }
      if (grouped.length >= limit && byKey.size >= limit) break;
    }
  ...
  function listTargetModuleState(moduleName = '', limit = 50, options = {}) {
    ...
    return targetModuleState
      .map((item) => {
        if (isClosedSynchronizedTargetRecord(item)) return item;
  ...
  function applyArtifactDecision(id, actionId, meta = {}) {
    const artifact = artifacts.find(item => item.id === id);
    if (!artifact) return { ok: false, status: 404, error: 'Automation review surface not found' };
    const lifecycle = getArtifactLifecycle(artifact);
    const action = (artifact.decisionActions || buildDecisionActions(artifact.type, artifact.refs)).find(item => item.id === actionId);
    if (!action) return { ok: false, status: 400, error: 'Decision action is not available for this review surface' };
    const operationKey = buildActionOperationKey(artifact, action, lifecycle, meta.actionKey || '');
    if (useIdempotencyKey(operationKey)) {
      return { ok: true, status: 200, duplicateSubmissionSuppressed: true, artifact: serializeArtifact(artifact), decision: artifact.lastDecision || null, targetSync: artifact.targetSync || null };
    }
    if (lifecycle.closed) {
    ...
    artifact.transitionVersion = (Number.isFinite(Number(artifact.transitionVersion)) ? Number(artifact.transitionVersion) : 0) + 1;
    ...
    addNotification(
      ...
      'success',
      {
        ...
        pendingReview,
        targetSync,
        openTargetPayload: targetSync.openTargetPayload,
        operationKey,
  ...
  function applyArtifactLifecycleAction(id, lifecycleActionId, meta = {}) {
    ...
    if (!action) return { ok: false, status: 400, error: 'Lifecycle action is not available for this review surface' };
    const lifecycle = getArtifactLifecycle(artifact);
    const operationKey = buildActionOperationKey(artifact, action, lifecycle, meta.actionKey || '');
    if (useIdempotencyKey(operationKey)) {
      return { ok: true, status: 200, duplicateSubmissionSuppressed: true, artifact: serializeArtifact(artifact), decision: artifact.lastDecision || null, targetSync: artifact.targetSync || null };
    ...
    addNotification(
      ...
      'success',
      {
        ...
        openTargetPayload: targetSync.openTargetPayload,
        operationKey,
        notificationType: 'lifecycle_updated'
      }
    );
