...
function createAutomationHub({ getSystemMode }) {
  ...
  function listArtifacts(limit = 100) { return artifacts.slice(0, limit).map(serializeArtifact); }
  ...
  function applyArtifactDecision(id, actionId, meta = {}) {
    ...
    artifact.decisionRequired = action.nextStatus === 'pending_review';
    ...
    return { ok: true, status: 200, artifact: serializeArtifact(artifact), decision: decisionEntry };
  }
