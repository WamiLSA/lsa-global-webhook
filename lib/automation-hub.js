...
const PHASE_3_GROUNDWORK = {
  ...
  implementedScope: [
    ...
    'Knowledge quick-capture and capture-assistant saves now emit Automation Hub triggers for formatting/review groundwork.',
    'Automation Hub API exposes a phase-state endpoint for Phase 2 stability and Phase 3 rollout visibility.'
  ],
  rolloutGuardrails: [
...
const WORKFLOW_DEFINITIONS = [
  ...
  },
  {
    ...
    name: 'Manual provider matching rerun',
    active: true,
    phase: 2,
    trigger: { type: 'manual_trigger', source: 'automation_hub_operator', automatic: false },
...
function buildTargetReference(artifact = {}, action = null) {
  ...
  if (type === 'provider_matching_results') {
    const providerId = compactRef(refs.providerId || refs.duplicateId, 'matching-result-set');
    const providerLabel = toOperatorReferenceLabel(refs.providerId || refs.duplicateId, 'Provider matching result set');
...
function buildTargetSync(artifact = {}, action = null, extras = {}) {
  ...
  return {
    ...
    syncConfirmation: syncState === 'synchronized'
      ...
      : `${action?.targetModule || artifact.module} is waiting for an staff decision before final sync to ${targetReference.destinationPanel}.`,
...
function createAutomationHub({ getSystemMode, persistencePath } = {}) {
  ...
  function createRunArtifact({ wf, runId, triggerType, payload, meta, startedAt }) {
    ...
    const artifact = {
      ...
      automationOrigin: meta.manual ? 'operator_triggered' : (wf.trigger?.automatic ? 'automatic_trigger' : 'system_trigger'),
