...
function buildTargetSync(artifact = {}, action = null, extras = {}) {
  ...
  return {
    ...
    reviewState,
    syncState,
    syncLabel: syncState === 'synchronized' ? 'Synchronized to target module' : 'Pending operator review',
...
function getLifecycleActions(artifact = {}) {
  ...
  if (safeArray(policy.reopenStatuses).includes(status)) {
    actions.push({
      id: 'reopen_review',
      label: 'Reopen for a new staff decision',
  ...
  if (safeArray(policy.reviseStatuses).includes(status)) {
    ...
    actions.push({
      id: 'revise_item',
      label: policy.reviseLabel || (nextStatus === 'rerun_requested' ? 'Request a rerun from this decision' : 'Send back for staff edits'),
      nextStatus,
