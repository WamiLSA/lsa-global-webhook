...
function createAutomationHub({ getSystemMode, persistencePath } = {}) {
  ...
  function isClosedSynchronizedTargetSync(record = {}) {
    return record.reviewState === 'closed' && record.syncState === 'synchronized' && !record.pendingReview;
  }

  function pruneDuplicateClosedTargetSync(record) {
    if (!isClosedSynchronizedTargetSync(record)) return;
    for (let index = targetModuleState.length - 1; index >= 0; index -= 1) {
      const item = targetModuleState[index];
      if (item === record) continue;
      if (item.key === record.key && item.artifactId !== record.artifactId && isClosedSynchronizedTargetSync(item)) {
  ...
  function syncTargetModuleState(artifact, decisionEntry) {
    ...
    } else if (isClosedSynchronizedTargetSync(nextRecord)) {
      const closedSameKeyIndex = targetModuleState.findIndex(item => item.key === nextRecord.key && isClosedSynchronizedTargetSync(item));
      if (closedSameKeyIndex >= 0) targetModuleState[closedSameKeyIndex] = nextRecord;
      else targetModuleState.unshift(nextRecord);
    } else {
      targetModuleState.unshift(nextRecord);
    }
    pruneDuplicateClosedTargetSync(nextRecord);
