...
function buildTargetReference(artifact = {}, action = null) {
  ...
  if (type === 'provider_extraction_review') {
    const recordId = compactRef(refs.providerId, pending ? 'pending official provider record' : 'provider record awaiting final id confirmation');
    ...
    return {
      ...
      destinationObjectName: refs.providerId ? `Provider record ${refs.providerId}` : 'Official provider record',
      destinationPanel: 'Provider Capture Assistant / Official Providers',
      destinationUrl: targetUrl,
      openTargetLabel: refs.providerId ? `Open provider record ${refs.providerId}` : 'Open provider record panel',
      sourceObjectId: documentId,
      sourceObjectType: 'Provider document/extraction draft',
      traceLabel: `Provider extraction ${documentId} → ${recordId}`,
  ...
  if (type === 'kb_formatting_review') {
    ...
    return {
      ...
      destinationObjectName: status === 'published' ? `Published KB record from capture ${captureId}` : `Knowledge capture ${captureId}`,
      destinationPanel: status === 'published' ? 'Knowledge Base publication panel' : 'Quick Capture / KB suggestion panel',
      destinationUrl: targetUrl,
      ...
      traceLabel: `KB formatting draft ${captureId} → ${recordId}`,
      verificationHint: pending
  ...
  if (type === 'inbox_helper_panel') {
    ...
    return {
      ...
      destinationObjectName: refs.threadId ? `Inbox thread ${refs.threadId}` : 'Inbox helper suggestion panel',
      ...
      traceLabel: `Inbox helper suggestion → thread ${threadId}`,
      verificationHint: pending
  ...
  if (type === 'duplicate_review_queue') {
    ...
    const stateName = status === 'kept_separate' ? 'kept-separate duplicate state' : status === 'merged' ? 'merged duplicate state' : 'duplicate review state';
    return {
      destinationRecordId: duplicateId,
      destinationObjectType: 'Provider Management duplicate state',
      destinationObjectName: `Provider duplicate ${stateName}`,
      destinationPanel: 'Duplicate Review Queue / Provider Capture Assistant',
      destinationUrl: targetUrl,
      ...
      traceLabel: `Duplicate review ${duplicateId} → ${stateName}`,
      verificationHint: pending
  ...
  if (type === 'provider_matching_results') {
    const providerId = compactRef(refs.providerId || refs.duplicateId, 'matching shortlist/result set');
    const objectName = status === 'confirmed' ? `Confirmed provider match ${providerId}` : status === 'shortlisted' ? `Provider shortlist ${providerId}` : 'Provider matching result set';
    return {
      ...
      traceLabel: `Provider matching run ${compactRef(artifact.runId, fallbackArtifact)} → ${objectName}`,
      verificationHint: pending
...
function buildTargetSync(artifact = {}, action = null, extras = {}) {
  ...
  return {
    ...
    syncConfirmation: syncState === 'synchronized'
      ? `${action?.targetModule || artifact.module} accepted ${statusLabel} from Automation Hub for ${targetReference.destinationObjectName}.`
      : `${action?.targetModule || artifact.module} is waiting for an operator decision before final sync to ${targetReference.destinationPanel}.`,
