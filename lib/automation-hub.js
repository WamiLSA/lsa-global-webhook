...
function buildTargetReference(artifact = {}, action = null) {
  ...
  if (type === 'provider_matching_results') {
    ...
    return {
      ...
      destinationUrl: targetUrl,
      openTargetLabel: status === 'confirmed' ? 'Open provider record' : 'Open results',
...
function createAutomationHub({ getSystemMode, persistencePath } = {}) {
  ...
  function getNotificationGroupKey(message, level, details = {}) {
    ...
    if (details.artifactId && notificationType !== 'automation_completed') return [notificationType, details.artifactId, details.nextStatus || details.statusLabel || 'state'].join(':');
