...
const INTENT_KEYWORDS = {
  ...
  payment_options: ["payment options", "payment", "paiement", "versement", "installment", "tranche", "pay by"],
  turnaround: ["delivery time", "turnaround", "délai", "delai", "processing time", "urgent", "rush"],
  requirements: ["requirements", "documents", "documents needed", "pièces", "pieces", "conditions", "needed"],
  refund_policy: ["refund", "refunds", "remboursement", "rembolso", "rimborso", "policy", "policies"],
  contact: ["contact", "phone", "email", "whatsapp", "reach you", "coordonnées", "coordonnees"],
...
const INTENT_PRIORITY = [
  "provider_matching", "fees", "duration", "schedule", "levels", "location", "registration", "payment_options", "turnaround", "requirements", "refund_policy", "contact", "format", "certificate"
];
...
const ENTITY_CATALOG = {
  ...
  german_course: { domain: "course", terms: ["german", "deutsch", "allemand", "tedesco", "aleman", "alemán", "german course"] },
  portuguese_course: { domain: "course", terms: ["portuguese", "portugais", "portugues", "portoghese", "portuguese course"] },
  chinese_course: { domain: "course", terms: ["chinese", "chinois", "chino", "china", "mandarin", "mandarim", "mandarín", "cinese", "chinese course"] },
  arabic_course: { domain: "course", terms: ["arabic", "arabe", "arabo", "árabe", "arabic course"] },
  ...
  localization_service: { domain: "localization", terms: ["localization", "localisation", "localisation service", "website localization", "l10n"] },
  certified_translation: { domain: "translation", terms: ["certified translation", "sworn translation", "traduction certifiee", "traduction certifiée"] },
  exam_prep_ielts: { domain: "exam_prep", terms: ["ielts", "ielts preparation", "ielts prep"] },
  exam_prep_toefl: { domain: "exam_prep", terms: ["toefl", "toefl preparation", "toefl prep"] },
  exam_prep_tcf: { domain: "exam_prep", terms: ["tcf", "tcf preparation", "tcf prep"] },
  exam_prep_tef: { domain: "exam_prep", terms: ["tef", "tef preparation", "tef prep"] },
  exam_prep_dele: { domain: "exam_prep", terms: ["dele", "dele preparation", "dele prep"] },
  exam_prep_goethe: { domain: "exam_prep", terms: ["goethe", "goethe preparation", "goethe prep"] },
  exam_prep_hsk: { domain: "exam_prep", terms: ["hsk", "hsk preparation", "hsk prep"] },
  registration_process: { domain: "registration", terms: ["registration process", "enrollment process", "how to register", "inscription"] },
  refund_policy: { domain: "policy", terms: ["refund policy", "policy", "absence policy", "support policy", "refund"] },
  douala_location: { domain: "location", terms: ["douala", "douala location", "address douala"] },
...
const DOMAIN_SOURCE_RULES = {
  ...
  localization: new Set(["kb_articles", "kb_capture_assistant", "kb_quick_capture", "providers"]),
  exam_prep: new Set(["kb_articles", "kb_capture_assistant", "kb_quick_capture"]),
  registration: new Set(["kb_articles", "kb_capture_assistant", "kb_quick_capture"]),
  policy: new Set(["kb_articles", "kb_capture_assistant", "kb_quick_capture"]),
  location: new Set(["kb_articles", "kb_capture_assistant", "kb_quick_capture", "providers"]),
...
const SUB_VARIANT_PRIORITY = ["accelerated", "intensive", "online", "in_person", "standard", "beginner", "advanced"];
...
async function runTokenizedOrSearch({
  ...
  const fallbackFieldSearch = async (term) => {
    for (const field of (fields || [])) {
      ...
      const wildcardToken = `%${term.replace(/\s+/g, "%")}%`;
      const { data, error } = await query.ilike(field, wildcardToken);
      if (error) continue;
      if (Array.isArray(data) && data.length) return { data, error: null };
    }
  ...
  for (const term of uniqueTerms) {
    ...
    let data = null;
    let error = null;
    ({ data, error } = await buildBaseQuery().or(orClause));
    if (error && /failed to parse logic tree|parse/i.test(String(error.message || ""))) {
      const fallback = await fallbackFieldSearch(term);
      data = fallback.data;
...
function detectEntity(query, recentContext = {}, intent = "general") {
  ...
  const dynamicCourse = normalized.match(/\b(italian|english|french|german|spanish|portuguese|arabic|chinese)\s+(course|cours|class|program)\b/i);
  if (dynamicCourse) {
    const language = normalizeForRetrievalTerms(dynamicCourse[1]).join(" ");
    return {
      entity: `${language}_course`,
      score: 1,
      domain: "course",
      terms: [dynamicCourse[0], language, "course"]
    };
  }
  if (isShortFollowUpQuery(query) && intent !== "general" && recentContext.topicLabel) {
    return {
      entity: recentContext.entity || "context_topic",
      score: 0,
      domain: recentContext.domain || null,
      terms: [recentContext.topicLabel].filter(Boolean)
    };
  }
...
function classifyRecordDomain(source, textBlob = "") {
  ...
  if (/\b(ielts|toefl|tcf|tef|dele|goethe|hsk|exam prep|exam preparation)\b/.test(normalized)) return "exam_prep";
  if (/\b(localization|localisation|l10n)\b/.test(normalized)) return "localization";
  if (/\b(registration|inscription|enrollment|enrolment|apply)\b/.test(normalized)) return "registration";
  if (/\b(refund|policy|absence|support)\b/.test(normalized)) return "policy";
  if (/\b(location|address|douala|city|branch|contact)\b/.test(normalized)) return "location";
...
function getCourseLanguageFromEntity(entityKey) {
  if (!entityKey || typeof entityKey !== "string") return null;
  const match = entityKey.match(/^([a-z]+)_course$/);
  return match?.[1] || null;
}

function hasConflictingCourseLanguage(recordBlob, expectedLanguage) {
  if (!expectedLanguage) return false;
  const mentions = detectCourseLanguageMentions(recordBlob);
  if (!mentions.size) return false;
  return [...mentions].some((language) => language !== expectedLanguage);
}

function detectRequestedSubVariant(query, recentContext = {}) {
  const normalized = normalizeForRetrievalTerms(query || "").join(" ");
  if (!normalized) {
    return {
      subVariant: recentContext.subVariant || null,
      correctionOverrideApplied: false,
      explicit: false
    };
  }
  ...
  for (const [variant, keywords] of Object.entries(SUB_VARIANT_KEYWORDS)) {
    ...
    for (const keyword of keywords) {
      const token = normalizeForRetrievalTerms(keyword).join(" ");
      if (!token) continue;
      if (normalized.includes(token)) score += token.includes(" ") ? 2 : 1;
    }
    if (score > 0) hits.push({ variant, score });
  }
  hits.sort((a, b) => b.score - a.score);
  const explicit = Boolean(hits.length);
  const correctionOverrideApplied = explicit && SUB_VARIANT_CORRECTION_PATTERN.test(normalized);

  if (explicit) {
    const prioritized = hits.sort((a, b) => {
      if (b.score !== a.score) return b.score - a.score;
      return SUB_VARIANT_PRIORITY.indexOf(a.variant) - SUB_VARIANT_PRIORITY.indexOf(b.variant);
    });
    return {
      subVariant: prioritized[0].variant,
      correctionOverrideApplied,
      explicit: true
    };
  }
...
function scoreRecord({
  ...
  if (requestedSubVariant) {
    const variantTerms = SUB_VARIANT_KEYWORDS[requestedSubVariant] || [];
    const hasVariantMatch = variantTerms.some((token) => {
      const normalizedToken = normalizeForRetrievalTerms(token).join(" ");
      if (!normalizedToken) return false;
      return titleLower.includes(normalizedToken) || categoryLower.includes(normalizedToken) || bodyLower.includes(normalizedToken);
    });
    if (hasVariantMatch) score += 70;
    else score -= 40;
  }
...
function createInternalRetriever({ supabase, detectLanguage }) {
  return async function retrieveInternalKnowledge(query, options = {}) {
    ...
    const subVariantDecision = detectRequestedSubVariant(rawQuery, options.contextMemory || {});
    const requestedSubVariant = subVariantDecision.subVariant;
    ...
    for (const result of sourceResults) {
      ...
      for (const record of (result.data || [])) {
        ...
        const requestedEntityCourseLanguage = getCourseLanguageFromEntity(detectedEntity?.entity);
        if (detectedEntity?.domain === "course" && requestedEntityCourseLanguage) {
          const recordCourseLanguages = detectCourseLanguageMentions(recordBlob);
          if (!recordCourseLanguages.has(requestedEntityCourseLanguage)) continue;
          if (hasConflictingCourseLanguage(recordBlob, requestedEntityCourseLanguage)) continue;
        }
        ...
        if (requestedSubVariant) {
          const normalizedBlob = normalizeForRetrievalTerms(recordBlob).join(" ");
          const variantTerms = (SUB_VARIANT_KEYWORDS[requestedSubVariant] || [])
            .map(item => normalizeForRetrievalTerms(item).join(" "))
            .filter(Boolean);
          const hasVariantHit = variantTerms.some(term => normalizedBlob.includes(term));
          if (!hasVariantHit) continue;
        }
        ...
        if (requestedSubVariant) reasons.push(`sub_variant:${requestedSubVariant}`);
        if (subVariantDecision.correctionOverrideApplied) reasons.push("override:correction");
    ...
    return {
      ...
      sub_variant: requestedSubVariant,
      correction_override_applied: subVariantDecision.correctionOverrideApplied,
      matches: ranked,
      debug: options.debug ? {
        allowed_sources: allowedSources,
        requested_sources: requestedSources,
        filtered_sources: filteredRequestedSources,
        detected_sub_variant: requestedSubVariant,
        correction_override_applied: subVariantDecision.correctionOverrideApplied,
