...
function detectEntity(query, recentContext = {}) {
  const normalized = normalizeForRetrievalTerms(query).join(" ");
  ...
  if (recentContext.entity && ENTITY_CATALOG[recentContext.entity]) {
    return {
...
function createInternalRetriever({ supabase, detectLanguage }) {
  return async function retrieveInternalKnowledge(query, options = {}) {
    ...
    const detectedEntity = detectEntity(rawQuery, options.contextMemory || {});
    ...
    if (filteredRequestedSources.includes("kb_articles")) {
      ...
      if (courseRelated && queryCourseLanguages.size) {
        const languageTerms = [...queryCourseLanguages]
          .flatMap(language => COURSE_LANGUAGE_KEYWORDS[language] || [])
          .map(escapeLikeTerm)
          .filter(Boolean)
          .slice(0, 20);
        if (languageTerms.length) {
          const languageOrParts = languageTerms.flatMap(term => [
            `title.ilike.%${term}%`,
            `question.ilike.%${term}%`,
            `answer.ilike.%${term}%`,
            `keywords.ilike.%${term}%`
          ]);
          articleQuery = articleQuery.or(languageOrParts.join(","));
        }
      }
